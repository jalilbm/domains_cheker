...
async def chatGPT_lucky_shot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"""give me 40 domain names{f' which are related to  {", ".join(topics)}' if lucky_shot_topics else ''}, which have the word "prompt" on them, and it must not be "promptly" or any sort of that, it must be only prompt, the domain names can only be .com or .io or .ai, the shorter they are the better they will be, the domain names must be valuable are might not been taken yet"""
    await chatGPT_guess(
        update=update,
        context=context,
        prompt_=prompt,
